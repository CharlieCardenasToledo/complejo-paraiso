...
export class CashRegisterService {
  ...
  // Signal para el estado de la caja
  private cashRegisterState = signal<{
  ...
  // Selectores públicos para el estado de la caja
  public currentRegister = signal<CashRegister | null>(null);
  ...
  constructor() {
    ...
    // Cargar el estado inicial de la caja
    this.loadCurrentCashRegister();
  }

  /**
   * Carga o recarga el estado de la caja abierta.
   */
  public loadCurrentCashRegister() {
    ...
    from(getDocs(q)).pipe(
      ...
      catchError(error => {
        ...
        // Aquí podrías manejar el error, por ejemplo, emitiendo un estado de error
        return of(null);
      })
    ).subscribe(register => {
      this.currentRegister.set(register);
      this.isRegisterOpen.set(!!register);
   ...
   */
  getCashRegisterHistory(status?: 'open' | 'closed' | 'all', limitCount = 10): Observable<CashRegister[]> {
    const cashRegisterCollection = collection(this.firestore, 'cash_registers');
 ...
 * @returns Observable<boolean> Éxito de la operación
 */
  syncOrdersWithTransactions(): Observable<boolean> {
